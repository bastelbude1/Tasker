...
class ConditionalExecutor(BaseExecutor):
    ...
    def execute_conditional_tasks(conditional_task, executor_instance):
        ...
        master_timeout = executor_instance.get_task_timeout(conditional_task)
        ...
        executor_instance.log(f"Task {task_id}: Executing {branch} branch with {len(tasks_to_execute)} tasks (sequential, timeout={master_timeout}s{retry_info})")
        ...
        for task in tasks_to_execute:
            ...
            if sleep_seconds > 0 and not executor_instance.dry_run:
                executor_instance.log(f"Task {task_id}-{result['task_id']}: Sleeping for {sleep_seconds} seconds...")
                sleep_completion_event = threading.Event()

                def sleep_callback():
                    try:
                        executor_instance.log_debug(f"Task {task_id}-{result['task_id']}: Sleep completed")
                    except Exception as e:
                        # Log the logging exception but don't let it prevent event signaling
                        try:
                            executor_instance.log(f"Task {task_id}-{result['task_id']}: Sleep callback logging failed: {e}")
                        except Exception:
                ...
                sleep_async(
                    sleep_seconds,
                    sleep_callback,
                    task_id=f"{task_id}-{result['task_id']}-sleep",
                    logger_callback=executor_instance.log_debug
                )

                # Wait with timeout to prevent indefinite blocking
                if not sleep_completion_event.wait(timeout=sleep_seconds + 5.0):
                    executor_instance.log_warn(f"Task {task_id}-{result['task_id']}: Sleep timer did not complete within timeout, proceeding")
