...
run_test_category() {
    ...
    for test in "${tests[@]}"; do
        ...
        if [[ "$test" == host_validation_* ]]; then
            ...
            test_passed=$(PATH="/home/baste/tasker/test_cases/bin:$PATH" timeout 90 "$tasker_script" "$test_file" -r >/dev/null 2>&1; echo $?)
        else
...
run_validation_fail_category() {
    ...
    for test in "${tests[@]}"; do
        ...
        if [[ "$test" == host_validation_* ]]; then
            # Host validation tests should NOT skip host validation and need PATH
            PATH="/home/baste/tasker/test_cases/bin:$PATH" "$tasker_script" "$test_file" -r >/dev/null 2>&1
        else
            # Other tests can skip host validation for speed
